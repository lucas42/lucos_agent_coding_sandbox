...
# The persona helper + function block is regenerated from scratch each run
# (the format may change between versions), but NEVER by truncating the
# whole file — lima.yaml's boot-time provisioning appends its own
# career-advisor entry to this same file before setup-repos.sh is ever run
# by hand, and a truncating write here silently destroyed it on every
# re-run. Instead, the block is delimited by marker comments so it can be
# replaced in place, leaving anything else in ~/.bash_aliases untouched.
# See lucas42/lucos_agent_coding_sandbox#104.
ALIASES_MARKER_BEGIN="# BEGIN lucos-managed persona functions (generated by setup-repos.sh -- do not edit by hand)"
ALIASES_MARKER_END="# END lucos-managed persona functions"

PERSONA_BLOCK=$(mktemp)
{
    echo "$ALIASES_MARKER_BEGIN"
    ...
    for PERSONA_FILE in "$HOME/.claude/agents/lucos-"*.md; do
        ...
        echo "${PERSONA_NAME}() { _lucos_persona ${PERSONA_NAME} \"\$@\"; }"
        echo "  [added] ${PERSONA_NAME}" >&2
        ALIASES_ADDED=$((ALIASES_ADDED + 1))
    done
    echo "$ALIASES_MARKER_END"
} > "$PERSONA_BLOCK"

if grep -qF "$ALIASES_MARKER_BEGIN" "$BASH_ALIASES_FILE"; then
    # Re-run: replace the existing managed block in place.
    awk -v begin="$ALIASES_MARKER_BEGIN" -v end="$ALIASES_MARKER_END" -v blockfile="$PERSONA_BLOCK" '
    ...
    # First run: append the managed block, preserving whatever is already
    # in the file (e.g. lima.yaml's career-advisor entry).
    cat "$PERSONA_BLOCK" >> "$BASH_ALIASES_FILE"
fi
rm -f "$PERSONA_BLOCK"
...
# Guarded (like lima.yaml's career-advisor entry) now that Step 7.5 no longer
# truncates the file on re-run -- without this check, re-running the script
# would append a duplicate tmux-claude block every time instead of leaving
# the existing one alone.
if ! grep -qF "alias tmux-claude=" "$BASH_ALIASES_FILE"; then
    echo "" >> "$BASH_ALIASES_FILE"
    echo "# Start (or re-attach to) a tmux session running Claude." >> "$BASH_ALIASES_FILE"
    echo "# Claude teammates use the tmux backend and inherit bypassPermissions from this session." >> "$BASH_ALIASES_FILE"
    echo "alias tmux-claude='tmux new-session -A -s main claude'" >> "$BASH_ALIASES_FILE"
fi

echo "$ALIASES_ADDED persona function(s) written to $BASH_ALIASES_FILE"
